...
Asks a running server to issue an admin token.

```bash
doppel token add --name ci --group admin --socket /tmp/doppel.sock
```

```
token `ci` issued to group `admin`, revision 4213...
it is in force now, and this is the only time it is shown:
3f2504e0-4f89-41d3-9a0c-0305e82c3301
```

The value is generated by the server, appended to the stored configuration,
and brought into force before the command answers -- so the token works on the
next request rather than after someone remembers to reload. It is printed
alone on the last line, so `doppel token add --name ci | tail -1` captures it
without a parser.

It is shown once. Nothing stores it in a form this command could read back:
the copy that exists is in the configuration, and that is the copy to guard.

`--group` defaults to `user`. That is deliberate and worth knowing before
scripting it: `user` carries no write access under the default access block,
so a token meant for administration needs `--group admin` spelled out. A
default that granted administration would make one forgotten flag an
escalation.

Socket resolution matches `config reload`: `--socket` wins and consults no
store, otherwise the path comes from the configuration.

Exit `1` if the name is already taken, if the store refuses the write, or if
the socket cannot be reached. Exit `2` if `--name` or `--group` is not a legal
name -- that is refused during argument parsing, before anything is sent.
