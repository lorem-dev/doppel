 ...
 *
 * It brings a YAML parser and a schema validator with it -- most of what that chunk
 * weighs -- so someone who edits with the form never fetches either.
 */
const YamlMode = lazy(() => import("../components/YamlMode"));
...
function forwardingSummary(proxy: ProxyConfig): string {
  const parts = [
    proxy.resolve?.type === "header" ? "by header" : "default proxy",
  ];
...
export default function ProxyFormPage() {
  ...
  /** Why the YAML cannot be saved, when it cannot. */
  ...
  const save = () => {
    ...
    if (editing) {
      if (!revision) {
        ...
        // answer 428 to.
        failed(
          new ApiError(0, "REVISION_REQUIRED", "this proxy was never loaded"),
        );
        return;
      }
  ...
  const patch = (fields: Partial<ProxyConfig>) =>
  ...
  return (
    <section className="flex flex-col gap-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/*
          A checkbox rather than two tabs: it is one document either way, and the
          question is only which way you would rather type it. It cannot be turned
          off while the YAML is broken -- the form shows the last document that
          parsed, so switching back would throw away whatever is being written.
        */}
        <label className="ml-auto flex items-center gap-2 text-sm text-slate-600 dark:text-slate-300">
          <input
            ...
            checked={asYaml}
            disabled={saving || (asYaml && broken !== undefined)}
            title={
              asYaml && broken ? `Fix the YAML first: ${broken}` : undefined
            }
            onChange={(event) => {
              setAsYaml(event.target.checked);
              setBroken(undefined);
            }}
          />
          Edit as YAML
      ...
      {asYaml ? (
        <Suspense fallback={<Spinner label="Loading the YAML editor..." />}>
          <YamlMode
            value={draft}
            schema={schema}
        ...
        <>
          ...
          <Field
            label="Type"
            info="type"
            hint="Only http. tcp is not implemented and the server refuses it."
          >
            <input
              className={controlClass}
              value={draft.type}
          ...
          <Field label="Upstream URL" info="url" error={errorFor("url")}>
            <input
              className={controlClass}
              value={draft.url}
              disabled={!allowed || saving}
          ...
          <Section title="Forwarding" summary={forwardingSummary(draft)}>
            <div className="flex gap-3">
              <Field
                ...
                error={
                  complaintFor("timeout", draft.timeout) ?? errorFor("timeout")
                }
              >
                <input
                  className={controlClass}
                  ...
                  disabled={!allowed || saving}
                  onChange={(event) =>
                    patch({ timeout: numberOr(event.target.value) })
                  }
                />
              </Field>
              <Field
                ...
                hint="Bytes, or 4Mi."
                error={
                  complaintFor("body_limit", draft.body_limit) ??
                  errorFor("body_limit")
                }
              >
                <input
            ...
            <div className="flex gap-2">
              <Field label="Resolve by" info="resolve.type">
                <select
                    ...
                    patch({
                      resolve:
                        event.target.value === "header"
                          ? {
                              type: "header",
                              header: draft.resolve?.header ?? "X-Proxy-Name",
                            }
                          : { type: "default" },
                    })
                  }
          ...
          <Section title="Faults" summary={faultSummary(draft)}>
            <div className="flex gap-3">
              <Field
                ...
                error={
                  complaintFor("replace", draft.replace) ?? errorFor("replace")
                }
              >
                <input
                  className={controlClass}
                  ...
                  disabled={!allowed || saving}
                  onChange={(event) =>
                    patch({ replace: numberOr(event.target.value) })
                  }
                />
              </Field>
              <Field
                label="Rewrite redirects"
                info="rewrite_redirects"
                hint="On by default: a Location pointing upstream is pointed back here."
              >
                <select
                  className={selectFullClass}
                  aria-label="Rewrite redirects"
                  value={
                    draft.rewrite_redirects === undefined
                      ? "default"
                      : String(draft.rewrite_redirects)
                  }
                  disabled={!allowed || saving}
                  onChange={(event) =>
                    patch({
            ...
            <div className="flex gap-3">
              <Field
                label="Loss rate"
                info="loss.percentage"
                hint="A fraction. Leave empty for none."
                error={
                  complaintFor("loss.percentage", draft.loss?.percentage) ??
                  errorFor("percentage")
                }
              >
                <input
              ...
              <Field
                ...
                info="loss.status"
                error={
                  complaintFor("loss.status", draft.loss?.status) ??
                  errorFor("status")
                }
              >
                <input
            ...
            <div className="flex gap-3">
              <Field
                label="Latency rate"
                info="latency.percentage"
                hint="A fraction of requests to delay."
                error={complaintFor(
                  "latency.percentage",
                  draft.latency?.percentage,
                )}
              >
                <input
                  className={controlClass}
              ...
              <Field
                ...
                info="latency.min"
                error={
                  complaintFor("latency.min", draft.latency?.min) ??
                  errorFor("min")
                }
              >
                <input
              ...
              <Field
                ...
                info="latency.max"
                error={
                  complaintFor("latency.max", draft.latency?.max) ??
                  errorFor("max")
                }
              >
                <input
          ...
          >
            {(draft.mocks ?? []).map((mock, index) => (
              <MockEditor
                ...
                onRemove={() => {
                  const mocks = (draft.mocks ?? []).filter(
                    (_, at) => at !== index,
                  );
      ...
      <div className="sticky bottom-0 -mx-4 mt-2 flex items-center gap-2 border-t border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        <Button
          ...
          reason={
            broken ??
            (editing
              ? "This token may not change this proxy"
              : "This token may not create proxies")
          }
        >
